...
def compatibility_graph(donors, recipients, k):
    donor_edges = []
    m = min(len(donors[0]) if donors else 0, len(recipients[0]) if recipients else 0)

    if k <= 0:
        return [list(range(len(recipients))) for _ in range(len(donors))]
    ...
    for d in range(len(donors)):
        ...
        for r in range(len(recipients)):
            counter = 0
            for j in range(m):
                if donors[d][j] == recipients[r][j]:
                    counter += 1
                    if counter >= k:
                        edges.append(r)
                        break
        donor_edges.append(edges)
